...
router.put('/:id', async (req, res) => {
  ...
  // 3. 결과 반환
  const userId = req.session.userId
  if(!userId)  res.status(401).json({message : '2You are not logged in!'});
...
router.delete('/:id', async (req, res) => {
  ...
  // 3. 204 상태코드 반환
  const userId = req.session.userId
  if(!userId)  res.status(401).json({message : '3You are not logged in!'});
...
router.post('/:postId/replies', async (req, res) => {
  ...
  const userId = req.session.userId
  if(!userId)  res.status(401).json({message : '4You are not logged in!'});
