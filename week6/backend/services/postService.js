...
async function updatePost(postId, title, content, userId) {
    ...
    if(post.userId !== userId) throw new HttpError('자신의 게시물만 수정할 수 있습니다!', 403);
    const createdPostId = await postRepository.update(postId, sanitizeHtml(title), sanitizeHtml(content), userId);
    if(!createdPostId) throw new HttpError('삭제에 실패했습니다..', 500);
    const updatedPost = await getPostById(createdPostId);
    return updatedPost;
}
