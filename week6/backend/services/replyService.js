...
async function getRepliesByPostId(postId) {
    ...
    if (!post)
        throw new HttpError('게시물을 찾을 수 없습니다!', 404)
...
async function createReply(content, postId, userId) {
    ...
    // 4. 생성된 댓글 조회 및 반환
    if(content.trim() === '')
        throw new HttpError("내용은 빈 칸일 수 없습니다.", 400)
...
async function deleteReply(replyId, userId) {
    ...
    if(!(await replyRepository.isOwner(replyId, userId)))
        throw new HttpError("자신의 댓글만 삭제할 수 있습니다!", 403)

    const isSuccessed = await replyRepository.deleteById(replyId)
    if(!isSuccessed)
        throw new HttpError("댓글을 삭제하는데 실패하였습니다.", 500)
