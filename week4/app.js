...
const getStudentCredit = async (student_name) => {
    const result = await runQuery('select group_concat(enrollments.grade) as grade from students natural join enrollments where students.student_name = ? ',
        [student_name])
    console.log(result[0].grade)
}

const getCourseStudents = async (course_name) => {
    const result = await runQuery('select group_concat(students.student_name) as name from enrollments natural join students natural join courses where courses.course_name = ?',
        [course_name])
