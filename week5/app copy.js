...
const getStudentCredit= async (student_name) => {
    const query = `SELECT GROUP_CONCAT(enrollments.grade) as grade from students NATURAL JOIN enrollments WHERE students.student_name = ?;`
...
const EnrollStudent = async (student_id, course_id, grade) => {
    const query = `INSERT INTO enrollments(student_id, course_id, grade) VALUES(?, ?, ?);`
    const query2 = `select students.student_name, courses.course_name, GROUP_CONCAT(enrollments.grade) from students 
                    INNER JOIN enrollments ON students.student_id = enrollments.student_id NATURAL JOIN courses where students_student_id = ?;`
    const regis = await runQuery(query, [student_id, course_id, grade])
    const result = await runQuery(query2, [student_id])
    console.log(result)
}
